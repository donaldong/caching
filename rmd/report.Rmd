...
# Workload Metrics 
There are 11 metrics associated with each chuck of the workload.

* `unique_size`: Number of unique addresses appeared in the chunk.
* `reads`: Number of memory reads.
* `writes`: Number of memory writes.
* `data`: Number of data access.
* `ins`: Number of instruction access.
...
## Unique Ratio
```{r}
plot.hist.metric(D, W, 'unqiue_ratio')
```
...
## Data Ratio
```{r}
plot.hist.metric(D, W, 'data_ratio')
```


## Instruction Ratio
```{r}
plot.hist.metric(D, W, 'ins_ratio')
```
